...
pub fn double_decoding_yields_simpleerror_when_resp_is_invalid_test() {
  ...
  parser.decode(resp)
  |> should.equal([parser.SimpleError(
    "Expected a floating point number after ','\r\n",
  )])
}
...
pub fn big_number_decoding_works_when_resp_is_valid_test() {
  ...
  parser.decode(resp)
  |> should.equal([parser.BigNumber(1_234_562_354_532)])
}
...
pub fn simple_string_decoding_fails_when_resp_is_invalid_test() {
  ...
  parser.decode(resp)
  |> should.equal([parser.SimpleError(
    "Protocol error. Expected a string after '+'",
  )])
}
...
pub fn array_decoding_works_when_resp_is_valid_test() {
  ...
  |> should.equal(
    [parser.Array(
      Some([parser.BulkString(Some("hey")), parser.BulkString(Some("echo"))]),
    ),]
  )
}
...
pub fn array_decoding_fails_when_array_size_greater_than_actual_size_test() {
  ...
  |> should.equal(
    [parser.Array(
      Some([
        parser.SimpleError("Protocol Error. Invalid type "),
        parser.BulkString(Some("hey")),
        parser.BulkString(Some("echo")),
      ]),
    )],
  )
}
